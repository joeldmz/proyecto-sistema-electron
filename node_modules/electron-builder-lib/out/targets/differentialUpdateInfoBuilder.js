...
let appendBlockmap = exports.appendBlockmap = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (file) {
        (_builderUtil || _load_builderUtil()).log.info({ file: (_builderUtil || _load_builderUtil()).log.filePath(file) }, "building embedded block map");
        return JSON.parse((yield (0, (_builderUtil || _load_builderUtil()).executeAppBuilder)(["blockmap", "--input", file, "--compression", "deflate"])));
    });
...
let createBlockmap = exports.createBlockmap = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (file, target, packager, safeArtifactName) {
        ...
        (_builderUtil || _load_builderUtil()).log.info({ blockMapFile: (_builderUtil || _load_builderUtil()).log.filePath(blockMapFile) }, "building block map");
        const updateInfo = JSON.parse((yield (0, (_builderUtil || _load_builderUtil()).executeAppBuilder)(["blockmap", "--input", file, "--output", blockMapFile])));
