...
exports.fpmPath = exports.SNAP_TEMPLATE_SHA512 = exports.SNAP_TEMPLATE_VERSION = undefined;
...
const SNAP_TEMPLATE_VERSION = exports.SNAP_TEMPLATE_VERSION = "0.2.0";
// noinspection SpellCheckingInspection
const SNAP_TEMPLATE_SHA512 = exports.SNAP_TEMPLATE_SHA512 = "2Uxlk/+BkZt5T4CePfi5Cbt35TLlCuO34M5kGaFeT/V1JCx5D6i+EAdMMp1AX9vi6/4zSKW/wB5Z+DZIaHacNg==";
...
function prefetchBuildTools() {
    ...
    return Promise.all([fpmPath.value, (0, (_binDownload || _load_binDownload()).getBinFromGithub)("snap-template", SNAP_TEMPLATE_VERSION, SNAP_TEMPLATE_SHA512)]);
}
function getZstd() {
    // noinspection SpellCheckingInspection
    return getTool({
...
function getTool(descriptor) {
    ...
    return (0, (_binDownload || _load_binDownload()).getBin)(`${name}-${version}-${process.arch}`, `https://github.com/${repository}/releases/download/${tagPrefix}${version}/${name}-v${version}-${platform.buildConfigurationKey}${archQualifier}.7z`, checksum).then(it => _path.join(it, `${name}${platform === (_core || _load_core()).Platform.WINDOWS ? ".exe" : ""}`));
}
